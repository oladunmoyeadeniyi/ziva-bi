...
function lineFromApi(apiLine: ExistingLine, categories: CategoryForForm[]): LineState {
  // Try to find GL name from category mappings
  let glName = "";
...
export default function EditExpensePage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        // Build line state: separate parent lines from split sub-lines
  ...
  const addSplitLine = (lineLocalId: string) => {
    setLines((prev) => prev.map((l) => l.localId === lineLocalId ? { ...l, split_lines: [...l.split_lines, makeSplit()] } : l));
    scheduleAutoSave();
  };
  ...
  const handleFileSelected = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file || !uploadTargetRef.current || !accessToken || !report_id) return;
    e.target.value = "";
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between mb-3">
          <div>
            ...
            {incompleteCount > 0 && <p className="text-xs text-amber-600 mt-0.5">{incompleteCount} line{incompleteCount !== 1 ? "s" : ""} incomplete</p>}
          </div>
          {canEdit && (
                ...
                <div className="flex items-center gap-3 px-4 py-3 bg-white">
                  ...
                  <div className="flex-1 min-w-0">
                    {cfg.coding_level === 0 ? (
                      ...
                      <button type="button" onClick={() => canEdit && setPickerFor({ lineLocalId: line.localId })}
                        disabled={!canEdit}
                    ...
                    <div className="space-y-3 mt-3">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                        <div>
                          ...
                          <input type="number" min="0.01" step="0.01" value={line.amount} disabled={!canEdit}
                            onChange={(e) => { updateLine(line.localId, { amount: e.target.value }); scheduleAutoSave(); }}
                            placeholder="0.00"
                            className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm text-right focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:bg-gray-50" />
                        </div>
                        <div>
                      ...
                      {cfg.coding_level === 2 && line.flag_incorrect && (
                        <div>
                          ...
                          <input type="text" value={line.flag_comment} disabled={!canEdit}
                            ...
                            className="w-full px-3 py-2 border border-amber-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-amber-400 disabled:bg-gray-50" />
                        </div>
                      )}
              ...
              return (
                <div key={line.localId} className="border border-gray-100 rounded-lg p-3">
                  <p className="text-xs font-semibold text-gray-600 mb-2">
                    Line {idx + 1} — {line.description || line.gl_number || "(line)"}
                  </p>
                  {lineDocs.length > 0 && (
                    <ul className="mb-2 space-y-1">
                      {lineDocs.map((doc) => (
                        <li key={doc.id} className="flex items-center gap-2 text-xs text-gray-700">
                          ...
                          {canEdit && <button type="button" onClick={() => handleDeleteDocument(doc.id)} className="text-red-400 hover:text-red-600 shrink-0">Remove</button>}
                        </li>
                      ))}
                    </ul>
                  )}
                  {canEdit && (
                    <button type="button" disabled={isUp} onClick={() => line.backendId && triggerUpload(line.backendId)}
