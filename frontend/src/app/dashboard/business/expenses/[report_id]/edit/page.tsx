 ...
 *
 * Pre-populates the form with the existing report's header and lines.
 * Supports DRAFT and REJECTED reports (REJECTED resets to DRAFT on save).
 *
 * M4 changes to the "Submit for Approval" flow:
...
export default function EditExpensePage() {
  ...
  const validate = (): string | null => {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (!l.amount || isNaN(amount) || amount <= 0) return `Line ${i + 1}: Amount must be a positive number.`;
    }
    return null;
  };
  ...
  const handleSubmitWithApprovers = async () => {
    ...
    const needsL2 =
      matrix.levels >= 2 &&
      (matrix.amount_threshold_l2 === null || total > parseFloat(matrix.amount_threshold_l2));
    const needsL3 =
      matrix.levels >= 3 &&
      (matrix.amount_threshold_l3 === null || total > parseFloat(matrix.amount_threshold_l3));
    ...
    setIsSubmitting(true);
    try {
      // Save header + lines first, then submit with approvers
      await saveHeaderAndLines();
      await apiFetch(`/api/approvals/reports/${report_id}/submit`, {
        method: "POST",
        token: accessToken!,
  ...
  const total = calcTotal(lines);
  const needsL2 = matrix
    ? matrix.levels >= 2 &&
      (matrix.amount_threshold_l2 === null || total > parseFloat(matrix.amount_threshold_l2))
    : false;
  const needsL3 = matrix
    ? matrix.levels >= 3 &&
      (matrix.amount_threshold_l3 === null || total > parseFloat(matrix.amount_threshold_l3))
    : false;
