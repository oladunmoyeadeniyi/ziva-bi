...
/**
 * New AR Invoice form — /dashboard/business/ar/invoices/new
 *
 * Creates a DRAFT AR invoice with multi-line support.
 * net_receivable = amount_base + vat_amount - wht_amount
 * (VAT increases what customer owes; WHT reduces what we actually receive)
 */
...
interface Customer { id: string; code: string; name: string; customer_type: string; credit_terms: string | null; }
...
/**
 * Compute derived amounts for one line.
 * netReceivable = amountBase + vatAmount - whtAmount
 * (AR: VAT adds to what customer owes; WHT reduces what we receive)
 */
function computeLine(line: LineInput, exchangeRate: number) {
  ...
  const netReceivable = amountBase + vatAmount - whtAmount;
  return { amountBase, vatAmount, whtAmount, netReceivable };
}
...
export default function NewArInvoicePage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      apiFetch<Customer[]>("/api/ar/customers", { token: accessToken }),
  ...
  const totals = lines.reduce(
    (acc, ln) => {
      ...
      return {
        ...
        netReceivable: acc.netReceivable + c.netReceivable,
      };
    },
    { amountBase: 0, vatAmount: 0, whtAmount: 0, netReceivable: 0 }
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const created = await apiFetch<{ id: string }>("/api/ar/invoices", {
        token: accessToken!,
        method: "POST",
        body: payload,
      });
      router.push(`/dashboard/business/ar/invoices/${created.id}`);
    } catch (e: unknown) {
  ...
  return (
    <PageContainer>
      <div className="flex items-center gap-3 mb-6">
        ...
        <PageHeading title="New AR Invoice" />
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1">Due Date (auto from credit terms)</label>
              <input
                type="date"
                value={dueDate}
            ...
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1">Service Period Start</label>
              <input
                type="date"
            ...
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1">Service Period End</label>
              <input
                type="date"
        ...
        <div className="bg-white border border-gray-200 rounded-lg overflow-hidden">
            ...
            <table className="w-full text-xs">
              <thead className="bg-gray-50 border-b border-gray-100">
                <tr>
                  ...
                  <th className="px-3 py-2 text-right font-medium text-gray-500 w-28">Amount ({currency})</th>
                  <th className="px-3 py-2 text-left font-medium text-gray-500 min-w-[160px]">Revenue GL</th>
                  ...
                  <th className="px-3 py-2 text-right font-medium text-gray-500 w-32">Net Receivable (NGN)</th>
          ...
          <div className="px-5 py-3 bg-gray-50 border-t border-gray-100">
            <div className="flex justify-end gap-8 text-xs">
              <div className="text-gray-500">Gross: <span className="font-medium text-gray-700">{formatMoney(totals.amountBase)}</span></div>
              ...
              <div className="text-gray-700 font-semibold">Net Receivable: <span className="text-indigo-700">{formatMoney(totals.netReceivable)}</span></div>
            </div>
          </div>
        </div>
