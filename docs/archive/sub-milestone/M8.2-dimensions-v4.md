# CC Brief — Dimensions Page v4: Source-First Form, Customer Order Standalone, Simplified Statistical IO
...
This brief supersedes the previous dimensions rebuild brief. Key changes:

1. **Source-first add form** — user picks value source first; the source drives
   what fields and options appear next. Not a flat form.

2. **Customer order is now a standalone dimension** — removed from Statistical IO
   hybrid. Statistical IO is now hybrid of employee + manual only.

3. **Org structure source** — when user picks org structure, they see a checklist
   of actual cost center nodes from the DB to include/exclude.

4. **Five predefined dimension types** with correct source badges.
...
If not already run from the previous dimensions brief, create a migration to add
`value_source`, `description`, and `icon` to `tenant_dimensions`. If those columns
already exist (previous brief was run), skip this.

```python
def upgrade() -> None:
    op.add_column("tenant_dimensions",
        sa.Column("value_source", sa.String(50), nullable=True,
...
### 2. New endpoint: GET /api/config/dimensions/org-structure-preview

Add this endpoint to the dimensions router. It returns the cost center nodes
from the org structure for display in the add form checklist:

```python
@router.get("/api/config/dimensions/org-structure-preview")
async def get_org_structure_preview(
    current_user: UserToken = Depends(require_auth),
    db: AsyncSession = Depends(get_db),
):
    """Return cost center nodes from org structure for dimension add form."""
...
### 3. Update dimension schemas and model

Same as previous brief — ensure `value_source`, `description`, `icon` are in
`DimensionCreate`, `DimensionUpdate`, and `DimensionResponse`.

Update CREATE and PATCH handlers to save and return these fields.
...
type ValuesSubTab = "employee" | "customer" | "manual";
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    ...
                    <button type="button" onClick={handleAdd}
                      disabled={addingDim || !addName || orgNodes.length === 0}
          ...
          {isLoading ? (
            ...
            <div className="space-y-2">
              {activeDims.map((dim, idx) => {
                ...
                const isAuto = isOrgAuto || isEmpAuto;
...
- The `customer_order` value_source is a new type — add it to the
  `SOURCE_LABELS` and `SOURCE_COLORS` maps as shown.
- The hybrid source for Statistical IO now has only TWO sub-sources:
  employee (auto) + manual. Customer category is removed from hybrid.
- The new `/api/config/dimensions/org-structure-preview` endpoint must be
  registered BEFORE the existing `/api/config/dimensions/:id` routes to avoid
  "org-structure-preview" being interpreted as a dimension ID.
- The "View values" button in the card footer sets `activeTab` to "values" and
  `selectedDimForValues` to the dimension's ID — this requires both state setters
  to be called together.
- The actual fetching and display of saved dimension values (the values list table)
  is NOT in this brief — the values tab shows the correct UI chrome but not data
  rows yet. That is a follow-up brief.
- Run `alembic upgrade head` only if the migration hasn't been run already.

---

## Files you are allowed to change:
1. `src/app/dashboard/business/settings/dimensions/page.tsx`
2. `app/models/master_data.py` (add fields if not already added)
3. `app/routers/` — dimension endpoints + new org-structure-preview endpoint
4. `app/schemas/` — dimension schemas if not already updated
5. Alembic migration file (only if columns don't exist yet)

## Commit message:
`feat: dimensions page v4 — source-first add form, customer order standalone, simplified statistical IO, org-structure checklist`
