...
- Standing rule: all real-write tests use the dedicated test tenant `f2aecfab-025f-410f-a7f6-df923172c8a1` ("PRAD — Test Tenant"), documented in `docs/TEST_TENANT.md` — NEVER live Red Bull.
...
4. Current "PRAD — Test Tenant" is NOT a real M9.0 shadow (no parent_tenant_id) — **kept deliberately** as a separate Ziva-internal engineering/QA fixture (not client-facing), distinct from per-client test shadows. Both concepts coexist for different audiences.
