...
/**
 * Currencies & FX Rates — dedicated tables version (FX milestone).
 * Replaces JSONB-backed currency setup with proper tenant_currencies + tenant_fx_rates.
 * Tabs: Enabled Currencies | FX Rates | Rate Lookup
 */
...
type Tab = "currencies" | "rates" | "lookup";
  ...
  return (
    <PageContainer>
      <PageHeading
        ...
        subtitle="Manage enabled currencies and exchange rates"
      />

      {/* Tabs */}
      <div className="flex gap-1 border-b border-gray-200 mb-6">
        {(["currencies", "rates", "lookup"] as Tab[]).map((t) => (
          <button
            key={t}
            onClick={() => setTab(t)}
            className={`px-4 py-2 text-sm font-medium capitalize transition ${tab === t ? "border-b-2 border-indigo-600 text-indigo-600" : "text-gray-500 hover:text-gray-700"}`}
          >
            {t === "currencies" ? "Enabled Currencies" : t === "rates" ? "FX Rates" : "Rate Lookup"}
          </button>
        ))}
      </div>
