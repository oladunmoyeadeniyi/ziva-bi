...
class TenantDetail(BaseModel):
    ...
    # SA-B-lite billing fields
    plan: str | None = None          # free | starter | growth | enterprise (NULL = free)
    paid_since: date | None = None   # date of first payment; NULL = not yet paying
    created_at: datetime
    updated_at: datetime
...
class CreateTenantRequest(BaseModel):
    ...
    A temporary password is auto-generated by the backend and returned in the
    response for one-time display. The admin must change it on first login.
    """
...
class CreateTenantResponse(BaseModel):
    ...
    admin_user_id: str
    admin_email: str
    temp_password: str  # one-time: generated by backend, shown once to SA
...
class BillingUpdate(BaseModel):
    """
    Body for PATCH /api/platform/tenants/{tenant_id}/billing.

    SA-B-lite: manual paid/plan flag — no payment provider integration.
    Both fields are optional; send only what you're changing.

    plan values: free | starter | growth | enterprise.
    paid_since: date the tenant first paid; set to None to clear.
    """

    plan: Optional[Literal["free", "starter", "growth", "enterprise"]] = None
    paid_since: Optional[date] = None


class BillingResponse(BaseModel):
    """Returned by PATCH /api/platform/tenants/{tenant_id}/billing."""

    id: str
    plan: str | None
    paid_since: date | None
    message: str
